...
export default function CategoryTabSelector({
  ...
  const tabItems = tabs.map((tab) => {
    ...
    return (
      <motion.button
        ...
        {...subtleMicroHover}
        className={`font-abyan-title font-normal bg-transparent border-none p-0 cursor-pointer whitespace-nowrap shrink-0 transition-colors duration-300 leading-normal ${
          size === "sm"
            ? "text-xs sm:text-sm"
  ...
  const rowClasses = `flex overflow-x-auto no-scrollbar flex-nowrap items-center w-full ${
    size === "sm"
      ? "justify-start gap-4 sm:gap-6 py-0.5"
      : "justify-start sm:justify-center gap-5 sm:gap-8 py-1"
  }`;
