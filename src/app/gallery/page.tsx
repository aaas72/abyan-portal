...
function GallerySkeleton() {
  return (
    <div className="min-h-screen bg-white text-slate-900 font-cairo">
      ...
      {/* Gallery Photo Cards Showcase Skeleton Grid (Diagonal Rounding: rounded-tr-[40px] rounded-bl-[40px]) */}
      <div className="max-w-5xl mx-auto px-4 grid grid-cols-1 sm:grid-cols-3 gap-6 py-6">
        {Array.from({ length: 6 }).map((_, i) => (
          <div
            key={i}
            className="h-60 bg-[#f0ebfe] border border-[#10b981]/10 animate-pulse p-4 flex flex-col justify-between rounded-tl-none rounded-br-none rounded-tr-[40px] rounded-bl-[40px]"
          >
